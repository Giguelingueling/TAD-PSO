...
class World(object):
    ...
    '''
    def __init__(self, lower_bound, upper_bound, swarm_size, fitness_function, swarm_confidence=2.0):
        ...
        self._swarm = Swarm(number_of_creatures=self._swarm_size, lower_bound=self._lower_bound,
                            upper_bound=self._upper_bound, random=self._random)
...
for repeat in range(nmbr_repetition):
    ...
    for number_of_dimensions in dimensions:
        ...
        swarmProcess = World(lower_bound=lower_bound, upper_bound=upper_bound, swarm_size=swarm_size,
